...
def feed_ramos(sh: Worksheet) -> dict:
    ...
    Código del ramo (llave primaria): {
        ...
        "descripcion": (list[str]) Descripciones obtenidas del ramo,
        "comentarios": (list[str]) Comentarios obtenidos del ramo,
    ...
    while sh[f"B{y}"].value is not None:
        ...
        res[codigo]["descripcion"].append(sh[f"D{y}"].value)
        res[codigo]["comentarios"].append(sh[f"F{y}"].value if sh[f"F{y}"].value is not None else "")
        res[codigo]["dificultad"].append(sh[f"E{y}"].value)
        if sh[f"G{y}"].value is not None:
